...
#define AI_SERVER_URL "http://10.10.53.17:5000/transcribe_raw"
// #define AI_SERVER_URL "http://DESKTOP-47HKGTF:5000/transcribe_raw"
...
void askAI() {
  Serial.println("\nSending rec1.wav to AI...");

  File file = LittleFS.open("/rec1.wav", "r");

  if (!file) {
    Serial.println("Failed to open /rec1.wav");
    return;
  }
  ...
  if (httpCode > 0) {
    ...
    if (!err) {
      ...
      if (type == "gpio") {
        ...
        if (cmd == "LED1 UP") {
          digitalWrite(GPIO1_PIN, LOW);//Inverse logic
          Serial.println("GPIO1 ON");
        }

        else if (cmd == "LED1 DOWN") {
          digitalWrite(GPIO1_PIN, HIGH);//Inverse logic
...
void loop() {
  ...
  if (
    lastButtonState == HIGH && currentState == LOW) {
    Serial.println();
